...
export default {
  ...
  methods: {
    async searchIban() {
      try {
        const response = await axios.get('https://fafabank-app-z7hg.onrender.com/customers/search-iban', {
          params: {
    ...
    async transferFunds() {
      ...
      try {
        const response = await axios.post('https://fafabank-app-z7hg.onrender.com/transactions', this.transfer, {
          headers: {
